...
import { CandyMachine, PublicKey, NftWithToken, Nft, Sft, SftWithToken } from "@metaplex-foundation/js";
import { useWallet } from "@solana/wallet-adapter-react";

export const CandyMachineProvider = ({ children }: { children: JSX.Element | JSX.Element[] }) => {
    const { metaplex } = useMetaplex();
    const wallet = useWallet();
    const [candyMachine, setCandyMachine] = useState<CandyMachine | undefined>(undefined);
    const [collection, setCollection] = useState<Nft | NftWithToken | Sft | SftWithToken | undefined>(undefined);
    ...
    return (
        <CandyMachineContext.Provider value={{ candyMachine: candyMachine ? candyMachine : null, collection, collectionMeta, setCandyMachine }} >
            {children}
